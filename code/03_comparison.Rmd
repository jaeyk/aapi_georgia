...
```{r}
county_names <- names(ga_counties_count)
county_counts <- ga_counties_count %>%
  summarize(across(everything(), mean, na.rm = T)) %>%
  t() %>%
  tibble()

county_counts$names <- county_names
names(county_counts)[1] <- "pct"  
...
county_text <- df %>%
  filter(str_detect(processed_text, paste(top_10_counties, collapse = "|"))) %>%
  pull(processed_text)

# create iterator over tokens
tokens <- space_tokenizer(county_text)
```
...
wv_main <- glove$fit_transform(tcm, n_iter = 1000, convergence_tol = 0.00001, n_threads = 8)
...
target <- word_vectors[top_10_counties[1], , drop = F]

cos_sim <- sim2(x = word_vectors, y = target, method = "cosine", norm = "l2")

head(sort(cos_sim[,1], decreasing = T), 20)
```