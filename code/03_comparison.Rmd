...
paste("the total number of documents:", nrow(df))

paste("the percentage of press releases:", mean(df$press_release)) # this pct is quite substantial

paste("the percentage of recaps:", mean(df$recap)) # this pct is pretty small 
```
...
# add week variable
df <- df %>%
  mutate(week = cut(ymd(date), breaks = "week"))
...
```{r}
stringdist(df$subject[1], df$subject[2], method = 'cosine', q = 1)
```
...
```{r}
comb_n %>%
  ggplot(aes(x = as.Date(week), y = n, col = source)) +
  ...
  geom_vline(xintercept = as.Date("2020-11-03"),
             ...
             size = 2) +
  theme_bw() +
  labs(title = glue("Monthly newsletter publication count, {min(year(comb_n$week))}-{max(year(comb_n$week))}"),
       x = "",
...
all %>%
  ...
  ggplot(aes(x = as.Date(week), y = value, col = type)) +
  geom_point(alpha = 0.3) +
  ...
  geom_vline(xintercept = as.Date("2020-11-03"),
             ...
             size = 2) +
  theme_bw() +
  labs(title = glue("Monthly newsletter publication count, {min(year(all$week))}-{max(year(all$week))}"),
       x = "",
